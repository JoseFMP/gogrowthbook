/*
GrowthBook REST API

GrowthBook offers a full REST API for interacting with the GrowthBook application. This is currently in **beta** as we add more authenticated API routes and features.  Request data can use either JSON or Form data encoding (with proper `Content-Type` headers). All response bodies are JSON-encoded.  The API base URL for GrowthBook Cloud is `https://api.growthbook.io`. For self-hosted deployments, it is the same as your API_HOST environment variable (defaults to `http://localhost:3100`). The rest of these docs will assume you are using GrowthBook Cloud.  ## Authentication  We support both the HTTP Basic and Bearer authentication schemes for convenience.  You first need to generate a new API Key in GrowthBook. Different keys have different permissions:  - **Personal Access Tokens**: These are sensitive and provide the same level of access as the user has to an organization. These can be created by going to `Personal Access Tokens` under the your user menu. - **Secret Keys**: These are sensitive and provide the level of access for the role, which currently is either `admin` or `readonly`. Only Admins with the `manageApiKeys` permission can manage Secret Keys on behalf of an organization. These can be created by going to `Settings -> API Keys`  If using HTTP Basic auth, pass the Secret Key as the username and leave the password blank:  ```bash curl https://api.growthbook.io/api/v1 \\   -u secret_abc123DEF456: # The \":\" at the end stops curl from asking for a password ```  If using Bearer auth, pass the Secret Key as the token:  ```bash curl https://api.growthbook.io/api/v1 \\ -H \"Authorization: Bearer secret_abc123DEF456\" ```  ## Errors  The API may return the following error status codes:  - **400** - Bad Request - Often due to a missing required parameter - **401** - Unauthorized - No valid API key provided - **402** - Request Failed - The parameters are valid, but the request failed - **403** - Forbidden - Provided API key does not have the required access - **404** - Not Found - Unknown API route or requested resource - **429** - Too Many Requests - You exceeded the rate limit of 60 requests per minute. Try again later. - **5XX** - Server Error - Something went wrong on GrowthBook's end (these are rare)  The response body will be a JSON object with the following properties:  - **message** - Information about the error 

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package models

import (
	"encoding/json"
	"fmt"
"github.com/JoseFMP/gogrowthbook/common"
)

// checks if the GetVisualChangeset200Response type satisfies the common.MappedNullable interface at compile time
var _ common.MappedNullable = &GetVisualChangeset200Response{}

// GetVisualChangeset200Response struct for GetVisualChangeset200Response
type GetVisualChangeset200Response struct {
	VisualChangeset VisualChangeset `json:"visualChangeset"`
	Experiment *Experiment `json:"experiment,omitempty"`
}

type _GetVisualChangeset200Response GetVisualChangeset200Response

// NewGetVisualChangeset200Response instantiates a new GetVisualChangeset200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetVisualChangeset200Response(visualChangeset VisualChangeset) *GetVisualChangeset200Response {
	this := GetVisualChangeset200Response{}
	this.VisualChangeset = visualChangeset
	return &this
}

// NewGetVisualChangeset200ResponseWithDefaults instantiates a new GetVisualChangeset200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetVisualChangeset200ResponseWithDefaults() *GetVisualChangeset200Response {
	this := GetVisualChangeset200Response{}
	return &this
}

// GetVisualChangeset returns the VisualChangeset field value
func (o *GetVisualChangeset200Response) GetVisualChangeset() VisualChangeset {
	if o == nil {
		var ret VisualChangeset
		return ret
	}

	return o.VisualChangeset
}

// GetVisualChangesetOk returns a tuple with the VisualChangeset field value
// and a boolean to check if the value has been set.
func (o *GetVisualChangeset200Response) GetVisualChangesetOk() (*VisualChangeset, bool) {
	if o == nil {
		return nil, false
	}
	return &o.VisualChangeset, true
}

// SetVisualChangeset sets field value
func (o *GetVisualChangeset200Response) SetVisualChangeset(v VisualChangeset) {
	o.VisualChangeset = v
}

// GetExperiment returns the Experiment field value if set, zero value otherwise.
func (o *GetVisualChangeset200Response) GetExperiment() Experiment {
	if o == nil || common.IsNil(o.Experiment) {
		var ret Experiment
		return ret
	}
	return *o.Experiment
}

// GetExperimentOk returns a tuple with the Experiment field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetVisualChangeset200Response) GetExperimentOk() (*Experiment, bool) {
	if o == nil || common.IsNil(o.Experiment) {
		return nil, false
	}
	return o.Experiment, true
}

// HasExperiment returns a boolean if a field has been set.
func (o *GetVisualChangeset200Response) HasExperiment() bool {
	if o != nil && !common.IsNil(o.Experiment) {
		return true
	}

	return false
}

// SetExperiment gets a reference to the given Experiment and assigns it to the Experiment field.
func (o *GetVisualChangeset200Response) SetExperiment(v Experiment) {
	o.Experiment = &v
}

func (o GetVisualChangeset200Response) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetVisualChangeset200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["visualChangeset"] = o.VisualChangeset
	if !common.IsNil(o.Experiment) {
		toSerialize["experiment"] = o.Experiment
	}
	return toSerialize, nil
}

func (o *GetVisualChangeset200Response) UnmarshalJSON(bytes []byte) (err error) {
    // This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"visualChangeset",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(bytes, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varGetVisualChangeset200Response := _GetVisualChangeset200Response{}

	err = json.Unmarshal(bytes, &varGetVisualChangeset200Response)

	if err != nil {
		return err
	}

	*o = GetVisualChangeset200Response(varGetVisualChangeset200Response)

	return err
}

type NullableGetVisualChangeset200Response struct {
	value *GetVisualChangeset200Response
	isSet bool
}

func (v NullableGetVisualChangeset200Response) Get() *GetVisualChangeset200Response {
	return v.value
}

func (v *NullableGetVisualChangeset200Response) Set(val *GetVisualChangeset200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableGetVisualChangeset200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableGetVisualChangeset200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetVisualChangeset200Response(val *GetVisualChangeset200Response) *NullableGetVisualChangeset200Response {
	return &NullableGetVisualChangeset200Response{value: val, isSet: true}
}

func (v NullableGetVisualChangeset200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetVisualChangeset200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


